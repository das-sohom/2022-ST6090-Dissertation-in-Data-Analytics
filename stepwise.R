...
roc.outer=auc.outer=st=cm=list()
...
thresholds=acc_for_box=auc_for_box=acc_for_box2=auc_for_box2=numeric(50)
...
for(r in 1:R){
  ...
  for (k in 1:K){
    ...
    reg.fwd = regsubsets(stg~., data=x.train, method="forward")
    R2adj = summary(reg.fwd)$adjr2
    R2adj.index = which.max(R2adj)
    mod = summary(reg.fwd)$which[R2adj.index,]
    len[a]=length((which(mod)[-1]-1))
    forward.matrix[k,(which(mod)[-1]-1)] = 1
    forward.matrix[k,-(which(mod)[-1]-1)] = 0
    forward.matrix1[a,(which(mod)[-1]-1)] = 1
    forward.matrix1[a,-(which(mod)[-1]-1)] = 0
  ...
  for(i in 1:ncol(forward.matrix)){
    if((sum(forward.matrix[,i])/nrow(forward.matrix)) >= 0.50){
