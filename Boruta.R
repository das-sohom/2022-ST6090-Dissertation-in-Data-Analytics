...
acc_for_box=auc_for_box=acc_for_box2=auc_for_box2=numeric(1)
forward.matrix1 = matrix(nrow = 50, ncol = ncol(one_four))
colnames(forward.matrix1) = colnames(one_four)
forward.matrix1 = forward.matrix1[,-82]
x=1
y=1
a=1
for(r in 1:R){
  ...
  for (k in 1:K){
    ...
    acc_for_box[x]=acc.rf[k]
    
    
    
    rf.p=predict(rf.out,x.test,'prob')[,2]
    auc.rf[k] = roc(x.test$stg, rf.p)$auc
    auc_for_box[x]=auc.rf[k]
    x=x+1
    
    
    boruta.train <- Boruta(stg~., data = x.train,maxRuns = 100, doTrace = 1)
    st[[k]]= which(boruta.train$finalDecision=='Confirmed')
    len[a]= length(which(boruta.train$finalDecision=='Confirmed'))
    
    if(length(which(boruta.train$finalDecision=='Confirmed'))==0){
      forward.matrix[k,] = 0
      forward.matrix1[a,] = 0
    } else{
      forward.matrix[k,which(boruta.train$finalDecision=='Confirmed')] = 1
      forward.matrix[k,-which(boruta.train$finalDecision=='Confirmed')] = 0
      forward.matrix1[a,which(boruta.train$finalDecision=='Confirmed')] = 1
      forward.matrix1[a,-which(boruta.train$finalDecision=='Confirmed')] = 0
      a=a+1
    }
  }
  ...
  for(i in 1:ncol(forward.matrix)){
    if((sum(forward.matrix[,i])/nrow(forward.matrix)) >= 0.20){
  ...
  for (k in 1:K){
    ...
    acc_for_box2[y]=acc.rf_final[k]
    
    rf.p_final=predict(rf.sel,x.test2,'prob')[,1]
    auc.rf_final[k] =roc(x.test2$stg, rf.p_final)$auc
    auc_for_box2[y]=auc.rf_final[k]
    
    roc=roc(x.test2$stg, rf.p_final)
    proc[[k]]=roc
    cm=caret::confusionMatrix(rf.pred_final,x.test2$stg, positive="stage4")
